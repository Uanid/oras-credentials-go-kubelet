...
var (
	...
	apiVersions = map[string]schema.GroupVersion{
		kubeletconfigv1alpha1.SchemeGroupVersion.String(): kubeletconfigv1alpha1.SchemeGroupVersion,
		kubeletconfigv1beta1.SchemeGroupVersion.String():  kubeletconfigv1beta1.SchemeGroupVersion,
		kubeletconfigv1.SchemeGroupVersion.String():       kubeletconfigv1.SchemeGroupVersion,
	}
)
...
// NewCredentialProviderPlugins is called from kubelet to register external credential provider
// plugins according to the CredentialProviderConfig config file.
func NewCredentialProviderPlugins(pluginConfigFile, pluginBinDir string) ([]credentialprovider.DockerConfigProvider, error) {
	if _, err := os.Stat(pluginBinDir); err != nil {
		if os.IsNotExist(err) {
			return nil, fmt.Errorf("plugin binary directory %s did not exist", pluginBinDir)
		}

		return nil, fmt.Errorf("error inspecting binary directory %s: %w", pluginBinDir, err)
	}
	...
	if len(errs) > 0 {
		return nil, fmt.Errorf("failed to validate credential provider config: %v", errs.ToAggregate())
	}

	var dockerConfigProvider []credentialprovider.DockerConfigProvider
	for _, provider := range credentialProviderConfig.Providers {
		...
		if _, err := os.Stat(pluginBin); err != nil {
			if os.IsNotExist(err) {
				return nil, fmt.Errorf("plugin binary executable %s did not exist", pluginBin)
			}

			return nil, fmt.Errorf("error inspecting binary executable %s: %w", pluginBin, err)
		}

		plugin, err := newPluginProvider(pluginBinDir, provider)
		if err != nil {
			return nil, fmt.Errorf("error initializing plugin provider %s: %w", provider.Name, err)
		}
		dockerConfigProvider = append(dockerConfigProvider, plugin)
	}
...
func newPluginProvider(pluginBinDir string, provider kubeletconfigv1.CredentialProvider) (*pluginProvider, error) {
