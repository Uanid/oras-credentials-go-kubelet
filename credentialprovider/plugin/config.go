...
func readCredentialProviderConfigFile(configPath string) (*kubeletconfigv1.CredentialProviderConfig, error) {
	if configPath == "" {
...
func decode(data []byte) (*kubeletconfigv1.CredentialProviderConfig, error) {
	...
	if gvk.Group != kubeletconfigv1.GroupName {
		return nil, fmt.Errorf("failed to decode CredentialProviderConfig, unexpected Group: %s", gvk.Group)
	}

	if internalConfig, ok := obj.(*kubeletconfigv1.CredentialProviderConfig); ok {
...
func validateCredentialProviderConfig(config *kubeletconfigv1.CredentialProviderConfig) field.ErrorList {
